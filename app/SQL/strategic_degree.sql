-- Create the database
CREATE DATABASE strategic_degree;
...
CREATE TABLE kpis (
    id SERIAL PRIMARY KEY,
    focus_matrix_id INT REFERENCES focus_matrix(id),
...
CREATE TABLE risks (
    ...
    business_id INT REFERENCES businesses(id),
    focus_matrix_id INT REFERENCES focus_matrix(id),
    risk_factor_id INT REFERENCES risk_factors(id),
    probability INT,
    impact INT,
    rating TEXT,
    mitigation TEXT
...
-- Ensure the database user 'root' has all privileges
GRANT ALL PRIVILEGES ON DATABASE strategic_degree TO root;
